...
function App() {
    ...
    async function fetchData() {
      try {
        const [cartResponse, favoriteItemsResponse, itemsResponse] = await Promise.all([
          ...
          axios.get('https://612a4354068adf001789baa2.mockapi.io/favoriteItems'),
  ...
  const addToCartClick = async (obj) => {
    try {
      ...
      if (findItem) {
        ...
        await axios.delete(`https://612a4354068adf001789baa2.mockapi.io/cart/${findItem.id}`);
      } else {
        setCartItems((prev) => [...prev, obj]);
        const { data } = await axios.post('https://612a4354068adf001789baa2.mockapi.io/cart', obj);
        setCartItems((prev) =>
          prev.map((item) => {
  ...
  const removeFromCartClick = async (id) => {
    try {
      ...
      await axios.delete(`https://612a4354068adf001789baa2.mockapi.io/cart/${id}`);
    } catch (error) {
  ...
  const addToFavoriteItemsClick = async (obj) => {
    try {
      ...
      if (findItem) {
        ...
        axios.delete(`https://612a4354068adf001789baa2.mockapi.io/favoriteItems/${findItem.id}`);
      } else {
